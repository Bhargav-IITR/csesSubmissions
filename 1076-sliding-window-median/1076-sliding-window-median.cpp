...
int minstRem(multiset <int> &mst){
    int num = *mst.begin();
    mst.erase(mst.find(num));
    return num;
}
 
int maxstRem(multiset <int> &mst){
    int num = *mst.rbegin();
    mst.erase(mst.find(num));
    return num;
}
 
void addNum(int id, vector <int> &a){
    minSt.insert(a[id]);
    int mn = minstRem(minSt);
    maxSt.insert(mn);
    if(maxSt.size() > minSt.size() + 1){
        //transfer an element-
        minSt.insert(maxstRem(maxSt));
    }
}
...
void removeNum(int num){
    // 1. Look for the number in maxSt first
    auto it = maxSt.find(num);
    if(it != maxSt.end()) {
        maxSt.erase(it);
    } else {
        // 2. If it's not in maxSt, it must be in minSt
        it = minSt.find(num);
        if(it != minSt.end()) {
            minSt.erase(it);
        }
    ...
    // 3. Re-balance the multisets to maintain your size constraint: maxSt <= minSt + 1
    // Case A: maxSt became too small
    if(maxSt.size() < minSt.size()) {
        maxSt.insert(minstRem(minSt));
    }
    // Case B: maxSt became too large
    else if(maxSt.size() > minSt.size() + 1) {
        minSt.insert(maxstRem(maxSt));
    }
}
...
void solve(){
    ...
    for(int i = 0 ; i < n ; i++){
        addNum(i, a);
        if(i >= k-1){
            // store answer for the k u collected -> remove the expired element to prepare for the next iteration
            ans.push_back(findMedian());
                int expiredEle = a[i-k+1];
            removeNum(expiredEle);
        }
    }
