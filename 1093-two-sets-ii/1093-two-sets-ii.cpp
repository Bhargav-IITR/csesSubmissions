...
ll func(ll num, ll left, ll n){
    if(left == 0) return 1;
    if(num > n) return 0;
    if(dp[num][left] != -1) return dp[num][left];
    // take - not take
    ll notTake = func(num+1, left, n);
    ll take = 0;
    if(num <= left) take = func(num+1, left-num, n);
    ll ans = (take+notTake)%M;
    return dp[num][left] = ans;
}
...
void solve(){
    ...
    if(totalSum%2) cout << 0 << endl;
        ...
        ll ans = func(1, targetSum, n);
        cout << (ans%M*MMI(2))%M  << endl;
    }
}
