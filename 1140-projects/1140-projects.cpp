...
ll findNextId(ll idStart , ll b, vector <vector <ll>> &projects){
    ll n = projects.size();
    ll lo = idStart, hi = n;
    while(lo < hi){
        ll mid = (lo+hi)>>1;
...
ll func(ll id, vector <vector <ll>> &projects){
    ll n = projects.size();
    if(id >= n) return 0;
    if(dp[id] != -1) return dp[id];
    ll b = projects[id][1], p = projects[id][2];
    // two options - take : not take
    ll notTake = func(id+1, projects);
    // ll nextId = id+1;
    // while((nextId < n) && (projects[nextId][0] <= b)){
    //     nextId++;
    // } -> convert this to a binary search
    ll nextId = findNextId(id+1, b, projects);
    ll take = p + func(nextId, projects);
...
void solve(){
    ...
    sort(projects.begin(), projects.end(), [&](vector <ll> a, vector<ll> b){
        if(a[0] < b[0]) return true;
        if(a[0] == b[0]) return (a[1] <= b[1]);
        return false;
    });
    memset(dp, -1, sizeof(dp));
    ll ans = func(0, projects);
    cout << ans << endl;
}   
