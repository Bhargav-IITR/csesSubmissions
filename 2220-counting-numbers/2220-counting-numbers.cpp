...
ll numbersInRange(ll id, ll prevDigit, ll tight, ll trailingZero, ll num){
    if(id == -1) return 1;
    if(dp[id][prevDigit][tight][trailingZero] != -1) return dp[id][prevDigit][tight][trailingZero];
    ll ans = 0;
    ll originalDigitAtId = (num / powTen[id]) % 10;
    ll limit = (tight) ? originalDigitAtId : 9;
    for(ll digit = 0 ; digit <= limit ; digit++){
        if(!trailingZero && (prevDigit == digit)) continue;
        ans += numbersInRange(id-1, digit, (tight && (digit == limit)), (trailingZero && (digit == 0)), num);
    }
    return dp[id][prevDigit][tight][trailingZero] = ans;
}
...
void solve(){
    ...
    ll ans1 = (b == 0) ? 1 : numbersInRange((ll)log10(b), 0, 1, 1, b);
    memset(dp, -1, sizeof(dp));
    ll ans2 = (a-1 == 0) ? 1 : numbersInRange((ll)log10(a-1), 0, 1, 1, a-1);
    cout << (ans1-ans2) << endl;
}   
 
int main() {
    for(int i = 0 ; i < 19 ; i++) powTen[i] = pow(10, i);
    solve();
}